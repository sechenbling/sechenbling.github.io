...
---
```
> The md file in '_post' directory has been set to `post` by default, so there is no need to add the variable layout in the Front Matter block.
{: .prompt-tip }
...
> The Jekyll tag `{ % highlight % }` is not compatible with this theme. But you should use `gem 'rouge'` to install dependency.
{: .prompt-danger }
